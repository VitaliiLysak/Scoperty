...
import { LOGGER } from '../services/logger/logger.provider';
...
export class NgxModalStack {
  ...
  public dismissAll(result: any) {
    for (let i = this.count - 1; i >= 0; i--) {
      try {
        ...
        this.logger.error('Failed to dismiss modal', error);
      }
  ...
  private register (modalRef: NgxModalRef) {
    ...
    this.makeChildModalActive(modalRef);
  ...
  private unregister (modalRef: NgxModalRef) {
    ...
    this.makeParentModalActive();

    if (this.count > 0) {
      const previous = this.get(this.count - 1).modal;
      if (!isNullOrUndefined(previous)) {
        this._currentIndex = Number.parseInt(previous.instance.zIndex, 10);
      }
    } else {
  ...
  private makeChildModalActive(modalRef): void {
    const modalStack = this._ngbModalRefStack;
    modalStack.forEach(childModalRef => {
      if (childModalRef.modal) {
        childModalRef.modal.instance.active = false;
      }
    });
    modalRef.modal.instance.active = true;
  }

  private makeParentModalActive(): void {
    const modalStack = this._ngbModalRefStack;
    const numberOfModals = modalStack.length;
    if (numberOfModals > 0) {
      const parentModalRef = this._ngbModalRefStack[(numberOfModals - 1)];
      if (parentModalRef.modal) {
        parentModalRef.modal.instance.active = true;
      }
    }
  }
}
